...
class DailyMonitoringBLM:
    ...
    def _ensure_history_coverage(self, code, csv_dir, start_date, end_date):
        ...
        if need_refresh:
            ...
            # 策略：Sina 稳定返回 3000+ 行；Eastmoney 在此环境会报 ConnectionError 或静默截断（返回恰好500行）
            try:
    ...
    def _precompute_momentum_scores(self, prices):
        """预计算所有日期的动量得分（numpy加速，避免逐日pandas开销）
        
        Returns:
            dict: {code: np.ndarray} 每个ETF的得分数组（索引对应prices行号）
        """
        ...
        for code in codes:
            ...
            for i in range(lookback, n_dates):
                ...
                # 1. 短期动量（20日收益率）
                if len(w) >= 20:
                    momentum_20d = w[-1] / w[-20] - 1.0
                else:
                    momentum_20d = w[-1] / w[0] - 1.0
                
                # 2. 长期动量（60日）— 实际window<60，走else
                if len(w) >= 60:
                    momentum_60d = w[-1] / w[-60] - 1.0
                else:
    ...
    def _build_momentum_series_from_precomputed(self, precomputed, date_idx):
        """从预计算结果构建pd.Series（供calculate_BLM_weights使用）"""
        return pd.Series({c: precomputed[c][date_idx] for c in precomputed})
    
    def _precompute_BLM_weights(self, prices, precomputed_momentum):
        """预计算所有日期的目标权重（numpy加速）
        
        Returns:
            dict: {code: np.ndarray} 各ETF每天的归一化目标权重
        """
        codes = list(prices.columns)
        n = len(prices)
        lookback = self.lookback_period
        top_n = self.top_n
        max_sw = self.constraints.get('max_single_weight', 0.40)
        base_weights = {c: self.etf_pool[c]['base_weight'] for c in codes}
        min_holding = self.constraints.get('min_holding', 0)
        
        # 结果数组
        result = {c: np.zeros(n, dtype=np.float64) for c in codes}
        
        for i in range(lookback, n):
            # 1. 收集得分
            scores = {c: precomputed_momentum[c][i] for c in codes}
            
            # 2. 过滤有效ETF（价格>0）
            valid = [c for c in codes if prices[c].iloc[i] > 0]
            if not valid:
                continue
            
            # 3. 计算 rank (percentile)
            valid_scores = [scores[c] for c in valid]
            n_valid = len(valid_scores)
            if n_valid == 0:
                continue
            
            # 手动计算 percentile rank (和 pandas rank(pct=True) 一致: average method)
            sorted_pairs = sorted(enumerate(valid_scores), key=lambda x: x[1])
            ranks = [0.0] * n_valid
            j = 0
            while j < n_valid:
                k = j
                while k < n_valid - 1 and sorted_pairs[k+1][1] == sorted_pairs[k][1]:
                    k += 1
                avg_rank = (j + k) / 2.0 + 1  # 1-based average rank
                for m in range(j, k+1):
                    ranks[sorted_pairs[m][0]] = avg_rank / n_valid
                j = k + 1
            
            rank_map = {valid[idx]: ranks[idx] for idx in range(n_valid)}
            ...
            for c in codes:
                ...
                if rank > 0.75:
                    adj = ((rank - 0.75) / 0.25) * 0.30
                elif rank > 0.60:
                    adj = ((rank - 0.60) / 0.15) * 0.15
                elif rank < 0.25:
                    adj = -((0.25 - rank) / 0.25) * 0.20
                elif rank < 0.40:
                    adj = -((0.40 - rank) / 0.15) * 0.10
                else:
                    adj = 0.0
                tw[c] = max(0.02, bw + adj)
            
            # 5. 正动量检查
            pos_count = sum(1 for c in valid if scores[c] > 0)
            if pos_count == 0:
                for c in codes:
                    tw[c] = 0.0
                # 全零跳过，result 已初始化为0
            ...
            # 6. 只保留top_n
            if top_n < len(valid):
                top_codes = sorted(valid, key=lambda c: scores[c], reverse=True)[:top_n]
                for c in codes:
    ...
    def calculate_BLM_weights(self, prices, date):
       ...
       逻辑：
        ...
        """
        # 计算动量得分
        if hasattr(self, '_precomputed_weights') and self._precomputed_weights is not None:
            idx = self._date_to_idx.get(date, -1) if hasattr(self, '_date_to_idx') else prices.index.get_loc(date)
            if idx < 0:
                idx = prices.index.get_loc(date)
            target_weights = pd.Series({c: self._precomputed_weights[c][idx] for c in self._precomputed_weights})
            # 信号日志仍需记录（如果有预计算动量得分）
            if self._record_signals and hasattr(self, '_precomputed_momentum') and self._precomputed_momentum is not None:
                momentum_scores = self._build_momentum_series_from_precomputed(self._precomputed_momentum, idx)
                valid_codes = [code for code in momentum_scores.index 
                              if pd.notna(prices.loc[date].get(code)) and prices.loc[date].get(code, 0) > 0]
                if valid_codes:
                    ms = momentum_scores[valid_codes]
                    mr = ms.rank(pct=True)
                    self.signals_log.append({
                        'date': date,
                        'momentum_scores': {k: v for k, v in ms.to_dict().items() if pd.notna(v)},
                        'momentum_ranks': {k: v for k, v in mr.to_dict().items() if pd.notna(v)}
                    })
            return target_weights
        
        if hasattr(self, '_precomputed_momentum') and self._precomputed_momentum is not None:
            idx = self._date_to_idx.get(date, -1) if hasattr(self, '_date_to_idx') else prices.index.get_loc(date)
            if idx < 0:
                idx = prices.index.get_loc(date)
            momentum_scores = self._build_momentum_series_from_precomputed(self._precomputed_momentum, idx)
...
触发条件检查
        ...
        Args:
            ...
            current_value: 当前价值
            entry_prices: 入场价
            rows_since_rebalance: 距上次调仓条数
        
        Returns:
            dict: 触发结果
        """
        # 冷却期检查
        if rows_since_rebalance < self.cooldown_days:
            return {
                'rebalance': False,
                'reason_type': 'cooldown',
                'reason': f"冷却期内({rows_since_rebalance}/{self.cooldown_days}条)",
        ...
        if rows_since_rebalance < self.cooldown_days and not severe_deviation:
            return {
                'rebalance': False,
                'reason_type': 'cooldown',
                'reason': f"冷却期内({rows_since_rebalance}/{self.cooldown_days}条)",
    ...
    def backtest(self, start_date, end_date, verbose=True, prices=None, return_details=True, record_signals=True):
        ...
        # 预计算所有日期动量得分（numpy加速）
        self._precomputed_momentum = self._precompute_momentum_scores(prices)
        # 预计算所有日期目标权重（numpy加速）
        self._precomputed_weights = self._precompute_BLM_weights(prices, self._precomputed_momentum)
        # 预建日期→行号映射（避免反复 get_loc 二分查找）
        self._date_to_idx = {d: i for i, d in enumerate(prices.index)}
        # 找到所有ETF都有数据的日期范围
        ...
        # 计算动量，只持仓前3只（使用预计算得分）
        first_idx = prices.index.get_loc(first_day)
        momentum_scores = self._build_momentum_series_from_precomputed(self._precomputed_momentum, first_idx)
        ...
        # 逐日回测
        # 预提取价格矩阵（避免反复 prices.loc[date]）
        price_matrix = prices.values  # shape: (n_dates, n_etfs)
        price_codes = list(prices.columns)
        code_to_col = {c: j for j, c in enumerate(price_codes)}
        price_dates = list(prices.index)
        
        for i, date in enumerate(trading_days):
            # 获取当前价格（numpy 快速索引）
            row_idx = self._date_to_idx.get(date, -1)
            if row_idx < 0:
                row_idx = prices.index.get_loc(date)
            price_row = price_matrix[row_idx]
            current_prices = {c: price_row[code_to_col[c]] for c in price_codes}
            ...
            # 计算当前持仓权重（用 dict 替代 pd.Series）
            if current_value > 0:
                current_weights_dict = {}
                for code in current_positions.keys():
                    price = current_prices.get(code, 0.0)
                    current_weights_dict[code] = (current_positions[code] * price) / current_value
            else:
                current_weights_dict = {code: 0.0 for code in self.etf_pool.keys()}
            ...
            if len(self.rebalance_history) > 0:
                last_row_index = self.rebalance_history[-1]['row_index']
                rows_since_rebalance = i - last_row_index
            else:
...
if __name__ == "__main__":
    ...
    print("=" * 75)
    print("优化策略详细回测（偏离度25% + 冷却20天，Stage7低回撤正式基线）")
    print("=" * 75)
    
    strategy = DailyMonitoringBLM(
        initial_capital=52000, trigger_deviation=0.25, signal_weight=0.2,
    ...
    with open(output_dir / 'backtest_data.json', 'w', encoding='utf-8') as f:
